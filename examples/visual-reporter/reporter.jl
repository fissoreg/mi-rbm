...
struct VisualReporter <: Boltzmann.EpochReporter
  every::Int
  pre::Dict{Symbol,Any}
...
function report(reporter::VisualReporter, rbm::AbstractRBM, epoch::Int, current_batch::Number, scorer::Function, X::Boltzmann.Mat, ctx::Dict{Any,Any})
  
  update_args!(reporter.args, Dict(:X => X), args_aliases(rbm), ctx; pre=reporter.pre)
  make_plots!(reporter.plots, reporter.args)
  plot(map(p -> p[:plot], reporter.plots)..., size = (1500, 1000))
  gui()
  frame(reporter.anim)
  
  # textual logging
  println("Epoch $epoch - Time: $current_batch, Score: $(scorer(rbm,X))")
end
