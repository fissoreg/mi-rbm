...
features = Dict(
  :ys => [:rbm],
  :transforms => [rbm -> reshape_mnist(G(rbm)')],
  :title => "Features",
  :ticks => nothing
)
...
reconstructions = Dict(
  :ys => [(:rbm, :X)],
  :transforms => [(rbm, X) -> (idxs = rand(1:ns, 30); reshape_mnist(hcat(X[1:d, idxs], generate(rbm, X[:, idxs])[1:d, :]), r = 6))], #reshape_mnist(generate(rbm, X[:,1:100], n_gibbs=1))],
